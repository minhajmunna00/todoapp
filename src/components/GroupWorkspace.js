...
export default function GroupWorkspace(){
    ...
    const getPDFsFromServer = () => {
        axios.get("http://localhost:8080/pdf").then(
            (response) => {
    ...
    */
    const addFiletoServer = (data) => {
        ...
        axios.post('http://localhost:8080/uploadPDF', formData, {
            headers: {
    ...
    return (
        <div className = "groupWorkspace">
            <header> 
                <h1>{groupName}</h1>
                <h2>{members}</h2>
                <Link to= "/group-settings"><Button variant="outline-secondary">Settings</Button></Link>
            </header> 
            ...
            <Button variant="outline-primary" onClick={showModal}>Upload</Button>
